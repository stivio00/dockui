...
- **Files** (`f` on a container row): filesystem explorer.
...
`f` on a container row opens a filesystem explorer for that container.
The title shows the container name, the first line the current path plus
status (`…` while loading, the error in red, else `N items`).
Directories render cyan with a `/` suffix and a `▸` caret, files show
their size (`13B`, `1.1M`). `j/k/g/G` navigate, Enter/`l` (or a second
click on the selected row) enters a directory, `←`/`h`/Backspace goes to
the parent, `r` reloads, `/` filters by name, Esc returns to the tree.

Listings come from the docker archive API, so they work on stopped
containers too. Volumes have no content API in docker — `f` on a volume
row explains that only containers can be browsed.
